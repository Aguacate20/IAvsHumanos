...
st.markdown("""
    ...
    div[data-testid="column"]:nth-child(1) img {
        ...
        height: auto;
        display: block;
        margin-right: 200px !important; /* Aumenta para mover "IA" más a la izquierda */
    ...
    div[data-testid="column"]:nth-child(3) img {
        ...
        height: auto;
        display: block;
        margin-left: 200px !important; /* Aumenta para mover "Humano" más a la derecha */
...
def main():
    if st.session_state.screen == "questionnaire":
        ...
        with col1:
            ...
            st.image(img_ai, caption=" ", use_column_width=False, width=400)
            st.markdown("<span class='slider-label'>IA</span>", unsafe_allow_html=True)
        with col3:
            response = requests.get("https://raw.githubusercontent.com/SebastianFullStack/images/main/Humano.png")
            img_human = Image.open(BytesIO(response.content))
            st.image(img_human, caption=" ", use_column_width=False, width=400)
